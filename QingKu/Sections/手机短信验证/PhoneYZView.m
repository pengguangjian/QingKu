...
//  QingKu
//
//  Created by mdb-losaic on 2019/9/3.
...
@interface PhoneYZView () <SelectCountryViewControllerDelegete>
{
    UILabel *lbselectcountry;
}
@end
@implementation PhoneYZView
...
-(void)drawUI
{
    ...
    [lbtop setText:[MDB_UserDefault getSetStringNmae:@"shoujiyanzhenma"]];
    ...
    [lbtopmin setText:[MDB_UserDefault getSetStringNmae:@"qingshuruyanzhenma"]];
    ...
    [fieldcode mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.height.equalTo(btselect);
        make.top.equalTo(fieldphone.mas_bottom).offset(10);
        make.right.equalTo(fieldphone).offset(-90);
    }];
    ...
    [viewline1 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(fieldcode);
        make.right.equalTo(fieldphone);
